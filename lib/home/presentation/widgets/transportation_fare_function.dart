...
double calculateAppointmentPricing(FormGroup form) {
  ...
  // 3. التعديل الجوهري: تحديد مفتاح السعر بناءً على السويتش مباشرةً (ريف أو مدينة)
  // مهما كانت المحافظة المختارة (إدلب، حلب، حمص، دمشق)
  final String locationKey = isRural ? 'rural' : 'city';

  // تقريب عدد الأشخاص لأقرب فئة أعلى
  ...
  // المجموع المالي يبدأ من الصفر لتطبيق الجمع التراكمي
  double totalPrice = 0.0;
  double transportFees = 0.0;

  // 4. إضافة سعر الكفرات الأساسي
  totalPrice += basicPrices[locationKey]?[targetGuests] ?? 0.0;

  // 5. جمع ركن الذكريات تراكمياً إذا تم تفعيله
  if (hasMemories) {
    totalPrice += memoriesPrices[locationKey]?[targetGuests] ?? 0.0;
  }

  // 6. جمع ركن الأمانات تراكمياً إذا تم تفعيله
  if (hasSafes) {
    totalPrice += safesPrices[locationKey]?[targetGuests] ?? 0.0;
  }

  // 7. جمع أجور النقل (السيارة) بناءً على المنطقة المحددة من أي ريف
  if (isRural && hasCar && ruralTransportPrices.containsKey(ruralLoc)) {
    transportFees = ruralTransportPrices[ruralLoc]!;
    totalPrice += transportFees;
  }

  // 8. حساب المبلغ المتبقي تلقائياً بعد طرح المدفوع
  final double paid =
      double.tryParse(form.control('paid').value.toString()) ?? 0.0;
  double restValue = totalPrice - paid;

  // 9. تحديث الحقول مباشرة في الـ Form لتعكس النتائج على الشاشة فورااااً
